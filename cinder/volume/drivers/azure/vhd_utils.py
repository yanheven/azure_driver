...
def generate_vhd_footer(size):
    ...
    # spec: https://technet.microsoft.com/en-us/virtualization/
    # bb676673.aspx#E3B
    # Field         Size (bytes)
    # Cookie        8
    # Features      4
    ...
    timestamp = bytearray.fromhex(
        hex(long(datetime.datetime.now().strftime("%s")) -
            946684800).replace('L', '').replace('0x', '').zfill(8))
    ...
    to_checksum_array = cookie + features + version + data_offset + \
        timestamp + creator_app + creator_version + creator_os + \
        original_size + current_size + disk_geometry + disk_type + \
        unique_id + saved_reserved
    ...
    blob_data = cookie + features + version + data_offset + timestamp + \
        creator_app + creator_version + creator_os + original_size + \
        current_size + disk_geometry + disk_type + checksum + unique_id + \
        saved_reserved
